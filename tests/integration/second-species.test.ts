/**
 * Integration tests for SecondSpecies counterpoint generation
 */
...
describe('SecondSpecies Integration', () => {
	const cMajorScale = [
	...
	describe('generateCounterpoint', () => {
		test('should generate correct number of notes (2:1 ratio)', () => {
			const cantusFirmus = [
			...
			expect(result.length).toBe(cantusFirmus.length * 2); // 2:1 ratio
		});
		...
		test('should work with different keys', () => {
			...
			for (const { cf, scale } of keys) {
				...
				const result = species.generateCounterpoint(cantusFirmus);
				expect(result.length).toBe(cantusFirmus.length * 2);
			}
		});

		test('should generate notes with length 2 (half notes)', () => {
			const cantusFirmus = [
				new Note(NoteType.Note_C4, 4),
				new Note(NoteType.Note_D4, 4),
				new Note(NoteType.Note_C4, 4),
